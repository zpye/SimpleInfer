...
#include "layer/hard_swish.h"

#include <algorithm>
#include <cmath>

static float HardSwishFunc(const float a, const float alpha, const float beta) {
    ...
    return (a * (a * alpha + beta));
}

TEST_CASE("Test HardSwish layer") {
    ...
    const float alpha = hardswish_layer.alpha_;
    const float beta  = hardswish_layer.beta_;
    CHECK_EQ(Status::kSuccess,
             hardswish_layer.Forward(input_tensor, output_tensor));
            ...
            for (int k = 0; k < shape[2]; ++k) {
                for (int l = 0; l < shape[3]; ++l) {
                    CHECK_FLOAT_EQ(output_eigen_tensor(i, j, k, l),
                                   HardSwishFunc(input_eigen_tensor(i, j, k, l),
                                                 alpha,
                                                 beta));
                }
            }
