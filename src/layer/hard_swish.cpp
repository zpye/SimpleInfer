...
DEFINE_LAYER_REGISTRY(HardSwish);

HardSwish::HardSwish() {}

HardSwish::~HardSwish() {}

Status HardSwish::Init(const pnnx::Operator* op) {
...
Status HardSwish::Validate() {
    ...
    if (!(IsSameDataType<float>(input_tensor_nodes_[0]->tensor.GetDataType()) &&
        ...
        LOG(ERROR) << "HardSwish::Validate fail ["
...
Status HardSwish::Forward(const Tensor& input, Tensor& output) {
    ...
    auto expr2 = input_eigen_tensor * expr1;

    output_eigen_tensor.device(*device) = expr2;
